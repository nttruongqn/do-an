  ...
  return new Promise(async (resolve, reject) => {
    try {
      let feedbacks = await db.Feedback.findAll({
        ...
        let feedbackConfirms = await db.Feedback.findAll({
          where: {
            status: true,
          },
        });
      let dateClient = helper.convertDateClient(feedbacks.createdAt);
      feedbacks.map((feedback)=>{
      feedback.setDataValue("date", dateClient);
      return feedback; 
      })
      feedbackConfirms.map((feedback) => {
         feedback.setDataValue("date", dateClient);
         return feedbackConfirms;
       });
